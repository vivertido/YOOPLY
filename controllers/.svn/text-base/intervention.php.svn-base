...
class Intervention extends MY_Controller
{
	...
	function _list($period, $filter, $school_id, $teacher_id = 0, $student_id = 0)
	{
		...
		$data['interventions'] = $this->Intervention_model->find($school_id, $teacher_id, $student_id, 0, 0, $start_time, $end_time, $expand_teacher, $expand_student);

		$this->load->model('Settings_model');
		$labels = json_decode($this->Settings_model->get_settings($school_id, 'labels'));
	
		$data['interventionlabel'] = $labels->interventions;
		$data['title_for_layout'] = trim($labels->interventions);
		...
		if($this->input->post('submit'))
		{
			...
			$filtered_list = array();
			foreach($data['interventions'] as $d)
			{
			...
			$data['interventions'] = $filtered_list;
			$this->layout->view('intervention/list_print', $data);
			return;
		}

		$this->layout->view('intervention/list', $data);
	}	
	...
	function add($student_id)
	{
		...
		if($this->input->post('submit'))
		{
			$reason = $this->input->post('intervention');
			...
			$this->load->model('Intervention_model');
			$reinforcement_id = $this->Intervention_model->create($school_id, $teacher_id, $student_id, $reason, $notes, $time_incident);
			...
			if(!empty($notify))
			{
				...
				$link = 'intervention/view/'.$reinforcement_id;
				$object_id = 'intervention/'.$reinforcement_id;
				$text = $student->firstname.' '.$student->lastname.' has received a '.$labels->intervention;
		...
		else
		{
			...
			$this->layout->view('intervention/add', array(
	...
	function student($student_id)
	{
		...
		$this->load->model('Intervention_model');
		$interventions = $this->Intervention_model->get_by_student($school_id, $student_id, $teacher_id);

		$this->layout->view('intervention/student', array(
			'interventions' => $interventions,
			'student' => $student,
			'title_for_layout' => $student->firstname.' '.$student->lastname
		));
	...
	function view($intervention_id)
	{
		$school_id = $this->session->userdata('schoolid');
		$user_id = $this->session->userdata('userid');

		$this->load->model('Intervention_model');
		$intervention = $this->Intervention_model->get_intervention($intervention_id);

		$this->load->model('User_model');
		$teacher = $this->User_model->get_user($intervention->teacherid);

		if(empty($intervention) || $intervention->status != INTERVENTION_STATUS_ACTIVE)
		{
			$this->layout->view('intervention/error_interventionnotfound');
			return;
		}

		$this->load->model('User_model');
		$student = $this->User_model->get_user($intervention->studentid);
		...
		switch($this->session->userdata('role'))
		{
			...
			break;
			case 'a':
				if($intervention->schoolid == $school_id)
				{
					$permission_denied = false;
				}
			break;
			case 's':
				if($intervention->studentid == $user_id)
				{
					$permission_denied = false;
				}
			break;
			case 'p':
				if($this->User_model->has_parent($intervention->studentid, $user_id))
				{
					$permission_denied = false;
				}
			break;				
		...
		if($permission_denied)
		{
			$this->layout->view('intervention/error_permissiondenied');
			return;
		}
		...
		$data = array(
			...
			'student' => $student,
			'title_for_layout' => 'Intervention',
			'teacher' => $teacher
		);
		
		$this->load->model('Consequence_model');
		$data['consequences'] = $this->Consequence_model->get_by_incident('intervention', $intervention->interventionid);

		$this->layout->view('intervention/view', $data);
	}

	function edit($intervention_id)
	{
		if(!($this->session->userdata('role') == 't' || $this->session->userdata('role') == 'a'))
		{
			redirect('login');
		...
		{
			$this->layout->view('teacher/error_interventionnotfound');
			return;
		}
		...
		if($this->input->post('submit'))
		{
			...
			$this->load->model('Intervention_model');
			$this->Intervention_model->update($intervention_id, $reason, $notes, $time_incident);

			array_push($notify, $intervention->teacherid);
			...
			$link = 'intervention/view/'.$intervention_id;
			$object_id = 'intervention/'.$intervention_id;
			$text = $teacher->firstname.' '.$teacher->lastname.' changed a '.$labels->intervention.' for '.$student->firstname.' '.$student->lastname;
			...
			redirect('intervention/view/'.$intervention_id);
		}
		else
		{
			$this->load->model('Settings_model');
			...
			$this->layout->view('intervention/edit', array(
				'interventions' => $interventions,
				'intervention' => $intervention,
				'title_for_layout' => $student->firstname.' '.$student->lastname
			));
	...
	function remove($intervention_id)
	{
		...
		if($this->input->post('submit'))
		{
			$this->Intervention_model->remove($intervention_id);
		...
		{
			$this->layout->view('intervention/remove', array(
				'intervention' => $intervention,
				'student' => $student,
				'title_for_layout' => 'Intervention',
				'teacher' => $teacher
